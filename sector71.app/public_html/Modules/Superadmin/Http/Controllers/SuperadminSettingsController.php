...
class SuperadminSettingsController extends Controller
{
    ...
    public function edit()
    {
        ...
        $default_values = [
            ...
            'SECTOR71PAY_API_KEY' => $is_demo ? null : env('SECTOR71PAY_API_KEY', env('PAYMENTKORO_API_KEY')),
            'SECTOR71PAY_SECRET_KEY' => $is_demo ? null : env('SECTOR71PAY_SECRET_KEY', env('PAYMENTKORO_SECRET_KEY')),
            'SECTOR71PAY_BRAND_KEY' => $is_demo ? null : env('SECTOR71PAY_BRAND_KEY', env('PAYMENTKORO_BRAND_KEY')),
            'SECTOR71PAY_CREATE_URL' => $is_demo ? null : env('SECTOR71PAY_CREATE_URL', env('PAYMENTKORO_CREATE_URL', 'https://pay.sector71.codes/api/payment/create')),
            'SECTOR71PAY_VERIFY_URL' => $is_demo ? null : env('SECTOR71PAY_VERIFY_URL', env('PAYMENTKORO_VERIFY_URL', 'https://pay.sector71.codes/api/payment/verify')),
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $system_settings = $request->only(['app_currency_id', 'invoice_business_name', 'email', 'invoice_business_landmark', 'invoice_business_zip', 'invoice_business_state', 'invoice_business_city', 'invoice_business_country', 'package_expiry_alert_days', 'superadmin_register_tc', 'welcome_email_subject', 'welcome_email_body', 'additional_js', 'additional_css', 'offline_payment_details', 'facebook_pixel', 'google_analytics_id', 'whatsapp_number', 'whatsapp_greeting']);
            ...
            $env_settings = $request->only(['APP_NAME', 'APP_TITLE',
                ...
                'SECTOR71PAY_API_KEY', 'SECTOR71PAY_SECRET_KEY', 'SECTOR71PAY_BRAND_KEY',
                'SECTOR71PAY_CREATE_URL', 'SECTOR71PAY_VERIFY_URL',
                'FLUTTERWAVE_SECRET_KEY', 'FLUTTERWAVE_ENCRYPTION_KEY', 'MAPBOX_ACCESS_TOKEN',
            ]);

            // Backward compatibility: keep legacy PAYMENTKORO_* vars in sync.
            $env_settings['PAYMENTKORO_API_KEY'] = $env_settings['SECTOR71PAY_API_KEY'] ?? null;
            $env_settings['PAYMENTKORO_SECRET_KEY'] = $env_settings['SECTOR71PAY_SECRET_KEY'] ?? null;
            $env_settings['PAYMENTKORO_BRAND_KEY'] = $env_settings['SECTOR71PAY_BRAND_KEY'] ?? null;
            $env_settings['PAYMENTKORO_CREATE_URL'] = $env_settings['SECTOR71PAY_CREATE_URL'] ?? null;
            $env_settings['PAYMENTKORO_VERIFY_URL'] = $env_settings['SECTOR71PAY_VERIFY_URL'] ?? null;
