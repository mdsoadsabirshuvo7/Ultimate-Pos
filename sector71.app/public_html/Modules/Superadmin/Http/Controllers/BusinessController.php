...
class BusinessController extends BaseController
{
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $html .= ' <button type="button" class="btn btn-warning btn-xs apply-template-btn" data-business-id="'.$row->id.'" data-business-name="'.e($row->name).'">'.__('superadmin::lang.apply_template').'</button>';
    ...
    public function create()
    {
        ...
        $business_templates = \Modules\Superadmin\Entities\BusinessTemplate::active()->orderBy('name')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $templateId = $request->input('business_template_id');
            if (!empty($templateId)) {
                $templateUtil = new \Modules\Superadmin\Utils\BusinessTemplateUtil();
                $templateUtil->applyTemplate($business->id, $templateId, $user->id);
            }
            DB::commit();
    ...
    /**
     * Apply template to existing business
     */
    public function applyTemplate(Request $request)
    {
        ...
        try {
            ...
            $templateUtil = new \Modules\Superadmin\Utils\BusinessTemplateUtil();
            $result = $templateUtil->applyTemplate($businessId, $templateId, auth()->id());

            if ($result['success']) {
                $output = ['success' => 1, 'msg' => __('superadmin::lang.template_applied_success')];
            } else {
                $output = ['success' => 0, 'msg' => $result['msg']];
            }
        } catch (\Exception $e) {
            \Log::emergency('File:' . $e->getFile() . 'Line:' . $e->getLine() . 'Message:' . $e->getMessage());
    ...
    public function getTemplates()
    {
        ...
        $templates = \Modules\Superadmin\Entities\BusinessTemplate::active()
            ->orderBy('sort_order')
