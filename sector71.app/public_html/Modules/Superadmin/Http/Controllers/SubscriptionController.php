...
class SubscriptionController extends BaseController
{
    ...
    /**
    * Redirect customer to Sector71 Pay checkout page.
     *
     * @return \Illuminate\Http\RedirectResponse
     */
    public function paymentKoroCheckout($package_id)
    {
        if (! auth()->user()->can('superadmin.access_package_subscriptions')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            $create_url = env('SECTOR71PAY_CREATE_URL', env('PAYMENTKORO_CREATE_URL', 'https://pay.sector71.codes/api/payment/create'));
            $success_url = action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'paymentKoroCallback'], [$package_id]);
            $cancel_url = action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'paymentKoroCallback'], [$package_id]);

            $payload = [
                'cus_name' => ! empty($business['name']) ? $business['name'] : ($user['username'] ?? 'Customer'),
            ...
            $response = $this->paymentKoroRequest($create_url, $payload);

            if (! empty($response['status']) && ! empty($response['payment_url'])) {
                return redirect()->away($response['payment_url']);
            }

            $message = ! empty($response['message']) ? $response['message'] : __('messages.something_went_wrong');

            return redirect()
                ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'pay'], [$package_id])
                ->with('status', ['success' => 0, 'msg' => $message]);
        } catch (\Exception $e) {
            \Log::emergency('PaymentKoro checkout error:'.$e->getMessage());
    ...
    /**
    * Handle Sector71 Pay callback and verify transaction.
     *
     * @return \Illuminate\Http\RedirectResponse
     */
    public function paymentKoroCallback(Request $request, $package_id)
    {
        if (! auth()->user()->can('superadmin.access_package_subscriptions')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            $transaction_id = $request->get('transactionId', $request->get('transaction_id'));
            ...
            $verify_url = env('SECTOR71PAY_VERIFY_URL', env('PAYMENTKORO_VERIFY_URL', 'https://pay.sector71.codes/api/payment/verify'));
            $verify = $this->paymentKoroRequest($verify_url, ['transaction_id' => $transaction_id]);
            $verify_status = strtoupper((string) ($verify['status'] ?? ''));

            if (in_array($verify_status, ['PENDING'])) {
                return redirect()
                    ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'index'])
                    ->with('status', ['success' => 1, 'msg' => __('superadmin::lang.waiting_for_confirmation')]);
            }

            if (! in_array($verify_status, ['COMPLETED', 'SUCCESS'])) {
                return redirect()
                    ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'pay'], [$package_id])
                    ->with('status', ['success' => 0, 'msg' => __('messages.something_went_wrong')]);
            }

            $exists = Subscription::where('payment_transaction_id', $transaction_id)
                ->where('paid_via', 'paymentkoro')
                ->exists();

            if (! $exists) {
                $meta = [];
                if (! empty($verify['metadata']) && is_array($verify['metadata'])) {
                    $meta = $verify['metadata'];
                } elseif (! empty($verify['meta_data']) && is_array($verify['meta_data'])) {
                    $meta = $verify['meta_data'];
                }

                $business_id = $meta['business_id'] ?? request()->session()->get('user.business_id');
                $user_id = $meta['user_id'] ?? request()->session()->get('user.id');
                $package_from_meta = $meta['package_id'] ?? $package_id;

                $this->_add_subscription($business_id, $package_from_meta, 'paymentkoro', $transaction_id, $user_id);
            }
            ...
            \Log::emergency('PaymentKoro callback error:'.$e->getMessage());
    ...
    /**
    * Perform Sector71 Pay API call.
     *
     * @param  string  $url
     * @param  array  $payload
     * @return array
     */
    protected function paymentKoroRequest($url, array $payload)
    {
        $headers = [
            'API-KEY: '.env('SECTOR71PAY_API_KEY', env('PAYMENTKORO_API_KEY')),
            'SECRET-KEY: '.env('SECTOR71PAY_SECRET_KEY', env('PAYMENTKORO_SECRET_KEY')),
            'BRAND-KEY: '.env('SECTOR71PAY_BRAND_KEY', env('PAYMENTKORO_BRAND_KEY')),
            'Content-Type: application/json',
        ];
        ...
        if (! is_array($decoded)) {
            throw new \Exception('Invalid Sector71 Pay response');
        }
