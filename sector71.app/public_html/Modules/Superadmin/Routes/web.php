...
Route::middleware('web', 'auth', 'language', 'AdminSidebarMenu', 'superadmin')->prefix('superadmin')->group(function () {
    ...
    Route::resource('/business-templates', Modules\Superadmin\Http\Controllers\BusinessTemplateController::class);
    Route::get('/business-templates/{id}/duplicate', [Modules\Superadmin\Http\Controllers\BusinessTemplateController::class, 'duplicate']);
    Route::post('/business/apply-template', [Modules\Superadmin\Http\Controllers\BusinessController::class, 'applyTemplate'])->name('business.apply-template');
    Route::get('/business/templates/get', [Modules\Superadmin\Http\Controllers\BusinessController::class, 'getTemplates'])->name('business.get-templates');
...
Route::middleware('web', 'SetSessionData', 'auth', 'language', 'timezone', 'AdminSidebarMenu')->group(function () {
    ...
    //Routes related to Sector71 Pay checkout
    Route::get('/subscription/{package_id}/payment-koro-checkout', [Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'paymentKoroCheckout'])->name('paymentKoroCheckout');
    Route::get('/subscription/{package_id}/payment-koro-callback', [Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'paymentKoroCallback'])->name('paymentKoroCallback');
